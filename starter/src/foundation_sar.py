...
class CustomerData(BaseModel):
    ...
    ssn_last_4: str = Field(..., description="Last 4 digits of SSN (e.g., '1234')")
    address: str = Field(..., description="Full mailing address")
    customer_since: str = Field(..., description="Customer onboarding date in YYYY-MM-DD format")
    risk_rating: Literal['Low', 'Medium', 'High'] = Field(..., description="Risk assessment rating")
    ...
    annual_income: Optional[int] = Field(None, description="Annual income in USD")

    # Validators
    @field_validator("date_of_birth", "customer_since")
    @classmethod
    def validate_date_format(cls, v, field):
        try:
            datetime.strptime(v, "%Y-%m-%d")
        except ValueError:
            raise ValueError(f"{field.name} must be in YYYY-MM-DD format")
        return v

    @field_validator("ssn_last_4")
    @classmethod
    def validate_ssn_last_4(cls, v):
        if not re.fullmatch(r"\d{4}", v):
            raise ValueError("SSN last 4 digits must be exactly 4 numeric digits")
        return v
...
class AccountData(BaseModel):
    ...
    customer_id: str = Field(..., description="Customer ID linked to this account like 'CUST_0001'")
    account_type: Literal['Checking', 'Savings', 'Money_Market'] = Field(..., description="Type of account")
    opening_date: str = Field(..., description="Date account was opened in YYYY-MM-DD format")
    current_balance: float = Field(..., description="Current account balance in USD (can be negative for overdrafts)")
    average_monthly_balance: float = Field(..., description="Average monthly balance in USD")
    status: Literal['Active', 'Closed'] = Field(..., description="Account status")

    # ✅ Date format validation
    @validator("opening_date")
    def validate_opening_date(cls, v):
        try:
            datetime.strptime(v, "%Y-%m-%d")
        except ValueError:
            raise ValueError("opening_date must be in YYYY-MM-DD format")
        return v

    # ✅ Balance validation
    @validator("average_monthly_balance")
    def validate_non_negative_average(cls, v):
        if v < 0:
            raise ValueError("average_monthly_balance must be non-negative")
        return v
...
class TransactionData(BaseModel):
    ...
    amount: float = Field(..., description="Transaction amount (can be negative for withdrawals)")
...
class CaseData(BaseModel):
    ...
    case_created_at: str = Field(default_factory=lambda: datetime.utcnow().isoformat(), description="ISO timestamp when case was created")
    data_sources: Dict[str, str] = Field(..., description="Source tracking dictionary")

    @field_validator("transactions")
    @classmethod
    def validate_non_empty_transactions(cls, value):
        if not value:
    ...
    @classmethod
    def validate_account_ownership(cls, accounts, info: ValidationInfo):
        customer = info.data.get("customer")
        if customer and any(acc.customer_id != customer.customer_id for acc in accounts):
            raise ValueError("All accounts must belong to the same customer")
        return accounts

    @field_validator("transactions")
    @classmethod
    def validate_transaction_linkage(cls, transactions, info: ValidationInfo):
        account_ids = {acc.account_id for acc in info.data.get("accounts", [])}
        if any(tx.account_id not in account_ids for tx in transactions):
...
class RiskAnalystOutput(BaseModel):
    ...
    classification: Literal['Structuring', 'Sanctions', 'Fraud', 'Money_Laundering', 'Other'] = Field(
        ..., description="Risk classification category"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score between 0.0 and 1.0"
    )
    reasoning: str = Field(
        ..., max_length=500, description="Step-by-step analysis reasoning (max 500 characters)"
    )
    key_indicators: List[str] = Field(
        ..., description="List of suspicious indicators found during analysis"
    )
...
class ComplianceOfficerOutput(BaseModel):
    ...
    narrative: str = Field(
        ..., max_length=1000,
        description="Regulatory narrative text (≤1000 characters, ~200 words)"
    )
    narrative_reasoning: str = Field(
        ..., max_length=500,
        description="Reasoning behind narrative construction (≤500 characters)"
    )
    ...
    completeness_check: bool = Field(
        ..., description="Whether the narrative meets all regulatory and procedural requirements"
    )
...
class DataLoader:
    ...
    def create_case_from_data(self, 
        ...
        try:
            # Step 2: Generate case_id
            case_id = str(uuid.uuid4())

            # Step 3: Create CustomerData object
            customer = CustomerData(**customer_data)

            # Step 4–5: Filter and validate accounts
            customer_accounts = [acc for acc in account_data if acc["customer_id"] == customer.customer_id]
            accounts = [AccountData(**acc) for acc in customer_accounts]
            account_ids = {acc.account_id for acc in accounts}

            # Step 6–7: Filter and validate transactions
            customer_txns = [txn for txn in transaction_data if txn["account_id"] in account_ids]
            transactions = [TransactionData(**txn) for txn in customer_txns]

            # Step 8–9: Create CaseData object
            timestamp = datetime.now(timezone.utc).isoformat()
            data_sources = {
                'customer_source': f"csv_extract_{datetime.now().strftime('%Y%m%d')}",
                'account_source': f"csv_extract_{datetime.now().strftime('%Y%m%d')}",
            ...
            self.logger.log_agent_action(
                ...
                reasoning="Case created from validated CSV fragments",
                execution_time_ms=execution_time_ms,
                success=True
            )
            ...
            self.logger.log_agent_action(
                ...
                output_data={},
                reasoning="Failed during case construction",
                execution_time_ms=execution_time_ms,
                success=False,
                error_message=str(e)
            )
